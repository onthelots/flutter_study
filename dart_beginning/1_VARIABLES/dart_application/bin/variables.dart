...
// late 키워드는, 특정 변수를 선언할 때, 값을 할당(초기화)를 나중에 미루기 때문에 초기화 되기 전까지는 해당 변수를 사용할 수 없도록 방지하는 역할임
...
// 예시로 들어보자. 여행을 간다고 하면 '여행지'를 결정하는 것은 이미 여행을 가기 전에 결정된 값이므로 이는 [const]값이 됨. 즉 컴파일시에 이미 값이 결정되어야 함
// 반면에, 여행을 간 이후에 점심을 먹을 곳을 결정하고자 함. 이는 이미 여행을 가고 난 후에 결정한 값이므로 [final]값이 됨. 런타임 시점에서 값이 결정되며, 한번 결정되었으므로 변경할 수 없음.
